...
Network = () ->
  ...
  tooltip = Tooltip("vis-tooltip", 100)
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      ...
      cutoff = d3.quantile(playcounts, 0.9)
  ...
  updateCenters = (artists) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(400).increment(20).keys(artists)
  ...
  showDetails = (d,i) ->
    content = '<p class="main" style="font-weight: bold;">' + d.name + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">'+ "Stage: " + d.artist + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">'+ "Email: " + d.Email + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">'+ "Status: " + d.Status + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">'+ "TP Status: " + d.TPStatus + '</span></p>'
...
$ ->
  ...
  $("#sub_Stages").on "change", (e) ->
    songFile = $(this).val()
    d3.json "data/#{songFile}", (json) ->
      myNetwork.updateData(json)

  $("#creation_list").on "change", (e) ->
    songFile = $(this).val()
    d3.json "data/#{songFile}", (json) ->
      myNetwork.updateData(json)
